...
import { FiCpu, FiTrendingUp, FiCheckCircle, FiChevronRight, FiAlertCircle } from 'react-icons/fi';
...
export function AIRecommendations() {
  ...
  const getHeuristicRecommendations = (pref: string, text: string): Recommendation[] => {
    const query = (pref + ' ' + text).toLowerCase();
    
    if (query.includes('defi') || query.includes('yield') || query.includes('pool')) {
      return [
        { id: 'uniswap', reason: 'High trade volume, robust automated liquidity protocol.' },
        { id: 'aave', reason: 'Decentralized lending leader, earn yield on collateral deposits.' }
      ];
    }
    if (query.includes('growth') || query.includes('volatility') || query.includes('l1') || query.includes('gain')) {
      return [
        { id: 'solana', reason: 'Extreme speed, cheap fees, and massive daily active users.' },
        { id: 'bitcoin', reason: 'Ultimate reserve asset, institutional adoption leader.' },
        { id: 'ethereum', reason: 'Industry-standard smart contract platform with gas burned on-chain.' }
      ];
    }
    if (query.includes('scalability') || query.includes('layer 2') || query.includes('l2')) {
      return [
        { id: 'arbitrum', reason: 'Ethereum layer-2 optimisitc rollup scaling standard.' },
        { id: 'optimism', reason: 'Developer friendly Ethereum scaling framework.' }
      ];
    }
    if (query.includes('oracle') || query.includes('real world') || query.includes('data')) {
      return [
        { id: 'chainlink', reason: 'Leading decentralized oracle protocol connecting code to API data feeds.' }
      ];
    }

    // Default
    return [
      { id: 'bitcoin', reason: 'Safe haven asset, high rating, gold standard.' },
      { id: 'ethereum', reason: 'Best platform for builders and smart contract developers.' }
    ];
  };
  ...
  return (
    <div className="bg-white dark:bg-[#141416]/90 border border-gray-100 dark:border-white/[0.06] rounded-3xl p-6 shadow-sm mb-8">
      <div className="flex items-center gap-2 mb-4">
        ...
        <div>
          <h2 className="font-extrabold text-sm text-gray-900 dark:text-white">AI Portfolio Recommendation Engine</h2>
          <p className="text-[10px] text-gray-400">Gemini analyzes our listings to suggest the best matching crypto assets.</p>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 p-2 bg-gray-50 dark:bg-on-surface rounded-xl border border-gray-100 dark:border-gray-800">
          <span className="text-[9px] font-bold text-gray-400 uppercase tracking-wider flex items-center gap-1">
            <FiAlertCircle className="w-3.5 h-3.5 text-primary" /> Optional: Set Gemini Key for real-time model reasoning
          </span>
          <input
            type="password"
            value={apiKey}
            ...
            className="px-3 py-1 rounded-lg border border-gray-250 dark:border-gray-700 text-[10px] outline-none bg-white dark:bg-[#141416]"
          />
        </div>
        ...
        >
          {loading ? 'Analyzing portfolio data...' : (
            <>
      ...
      {recommendedItems.length > 0 && (
        <div className="mt-6 border-t border-gray-100 dark:border-gray-800 pt-6 space-y-4">
          <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider">Recommended Assets</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recommendedItems.map((item) => (
              <div key={item.id} className="p-4 rounded-2xl border border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-[#1d1d22]/30 flex flex-col justify-between h-[160px]">
                <div className="flex gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-400 font-bold uppercase">{item.category} • ${item.price}</p>
                    <p className="text-[11px] text-gray-500 mt-1 line-clamp-2">{item.reason}</p>
                  </div>
                </div>
                
                <div className="flex justify-end pt-2">
                  <Link href={`/markets/${item.id}`} className="text-xs text-primary font-bold hover:underline flex items-center gap-0.5">
