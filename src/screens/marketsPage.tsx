  ...
  const toggleFavorite = (symbol: string, e: React.MouseEvent) => {
    ...
    if (favorites.includes(symbol)) {
      ...
      notifyWarning(
        ...
        `${symbol}/USDT has been removed from your watchlist.`,
      );
    } else {
      updated = [...favorites, symbol];
      notifySuccess(
        "Saved Successfully",
        `${symbol}/USDT has been added to your favorites watchlist.`,
      );
    }
      ...
      <section className="relative w-full bg-slate-50 dark:bg-[#141416]/50 py-12 overflow-hidden select-none border-b border-gray-100 dark:border-white/[0.03]">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col justify-center text-left">
          ...
          <p className="text-sm font-semibold text-secondary dark:text-gray-400">
            ...
            <span className="text-primary font-bold">$1.86T</span>
          </p>
        </div>
      </section>
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-base font-bold text-gray-900 dark:text-white tracking-tight mt-0.5">
                      USD{" "}
                      {price > 0
                ...
                <div className="text-right shrink-0">
                  <span
                    className={`text-xs font-bold px-2.5 py-1 rounded-full ${isPositive ? "bg-green-500/10 text-green-500" : "bg-red-500/10 text-red-500"}`}
                  >
                    {isPositive ? "+" : ""}
                    {change.toFixed(2)}%
                  </span>
        ...
        <div className="bg-white dark:bg-[#141416]/90 border border-gray-100 dark:border-white/[0.05] rounded-3xl p-6 shadow-sm space-y-5">
                ...
                <TableBody>
                  {paginatedItems.map((item, idx) => {
                    ...
                    return (
                      <TableRow
                        key={item.id}
                        className="transition-colors hover:bg-slate-50/50 dark:hover:bg-white/[0.01]"
                      >
                        {/* Index + Star */}
                        <TableCell className="text-center">
                          <div className="flex items-center justify-center gap-2 select-none">
                            ...
                            >
                              {favorites.includes(
                                item.title.split(" ")[0].toUpperCase(),
                              ) || favorites.includes(item.id.toUpperCase()) ? (
                                <FaStar className="w-3.5 h-3.5 text-amber-400 fill-amber-400 drop-shadow-[0_0_6px_rgba(251,191,36,0.6)]" />
                              ) : (
                                <FiStar className="w-3.5 h-3.5 text-gray-400 hover:text-amber-400" />
                              )}
                            </button>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-3">
                            ...
                            <div className="flex flex-col">
                              <span className="text-xs font-bold text-gray-950 dark:text-white leading-tight">
                                {item.title}
                              </span>
                        ...
                        {/* Last Price */}
                        <TableCell className="text-right font-bold text-xs text-gray-950 dark:text-white">
                          $
                          {price >= 1000
                            ? price.toLocaleString(undefined, {
                                minimumFractionDigits: 2,
                        ...
                        {/* 24h Change */}
                        <TableCell
                          className={`text-right font-bold text-xs ${isPositive ? "text-green-500" : "text-red-500"}`}
                        >
                          {isPositive ? "+" : ""}
                          {change.toFixed(2)}%
                        </TableCell>
                        ...
                        <TableCell>
                          <div className="flex items-center justify-center">
                            <svg
                              className="w-[100px] h-[28px] overflow-visible"
                              viewBox="0 0 120 40"
                            >
                              <path
                                d={sparkline}
                        ...
                        <TableCell className="text-center">
                          <Button
                            ...
                            size="sm"
                            className={`font-bold rounded-full px-5 py-1 text-[10px] cursor-pointer ${
                              idx === 1
          ...
          {totalPages > 1 && (
            <ReactPaginate
              ...
              pageLinkClassName="w-9 h-9 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center border border-gray-200 dark:border-gray-800 bg-white dark:bg-on-surface text-gray-700 dark:text-gray-200 hover:bg-slate-50 dark:hover:bg-gray-800 select-none"
              activeClassName="active-page"
              activeLinkClassName="!bg-primary !text-white !border-primary shadow-md shadow-primary/20"
              previousClassName="list-none"
              previousLinkClassName="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-on-surface text-xs font-bold text-gray-700 dark:text-gray-200 hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors select-none"
              nextClassName="list-none"
              nextLinkClassName="px-4 py-2 rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-on-surface text-xs font-bold text-gray-700 dark:text-gray-200 hover:bg-slate-50 dark:hover:bg-gray-800 transition-colors select-none"
