...
export default function AddItemPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(parsedPrice) || parsedPrice <= 0) {
      const errMsg = "Please enter a valid price greater than 0.";
      setError(errMsg);
      notifyError("Action Required", errMsg);
      return;
    }
    if (isNaN(parsedRating) || parsedRating < 1 || parsedRating > 5) {
      const errMsg = "Please enter a rating between 1.0 and 5.0.";
      setError(errMsg);
      notifyError("Action Required", errMsg);
      return;
    }
    ...
    try {
      ...
      notifySuccess("Saved Successfully", `Custom asset "${title}" created successfully.`);
      // Redirect to markets list
      router.push("/markets");
    } catch (err: any) {
      const msg = err?.message || "An error occurred while creating the asset.";
      setError(msg);
      notifyError("Error Occurred", msg);
      setSubmitting(false);
    }
  };
      ...
      <div className="max-w-2xl mx-auto px-6">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-xs text-gray-400 dark:text-gray-500">
            Add a new token, DeFi pool, or index to the listing explore
            platform.
          </p>
        </div>
